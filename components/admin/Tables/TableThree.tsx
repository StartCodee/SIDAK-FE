...
const { toast } = useToast();
  ...
  const acceptSupply = async (id: number) => {
	axios
		.put(
			`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/supply/neraca/approved/${id}`,
			{},
			{
				headers: {
  ...
  const columns: ColumnDef<Komoditas[]>[] = [
		...
		},
    {
      ...
      accessorKey: 'jumlah_kebutuhan',
    },
    {
      header: 'Jumlah Ketersediaan',
      accessorKey: 'jumlah_ketersediaan',
    },
		{
			header: 'Komoditas',
		...
		{
			header: 'Kabupaten',
			accessorKey: 'kabupaten_kota_name',
		},
		{
			header: 'Status',
  ...
  const getKomoditas = async () => {
    try {
			const response = await axios.get(
				`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/supply/neraca?type=unapproved`,
				{
					headers: {
						'content-type': 'application/json',
						Authorization: `Bearer ${Cookies.get('token')}`,
