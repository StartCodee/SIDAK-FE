...
export default function Home() {
    ...
    const getHargaPanganData = async (page: number = 1, limit: number = 2, start_date: string, end_date: string, komoditas: string, kota: string, kecamatan: string) => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/supply/harga-pangan?start_date=${start_date}&end_date=${end_date}&komoditas=${komoditas}&kabupaten_kota_id=${kota}&kecamatan_id=${kecamatan}`, {
                headers: {
    ...
    const getNeracaPanganData = async (page: number = 1, limit: number = 2, start_date: string, end_date: string, komoditas: string, kota: string, kecamatan: string) => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/supply/neraca-pangan?start_date=${start_date}&end_date=${end_date}&komoditas=${komoditas}&kabupaten_kota_id=${kota}&kecamatan_id=${kecamatan}`, {
                headers: {
                    'content-type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                },
    ...
    const getKecamatanOption = async (page: number = 1, limit: number = 2, kabupatenId: string) => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/kecamatan?kabupaten_id=${kabupatenId}`, {
                headers: {
                    'content-type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`,
                }
    ...
    const getKecamatanOptionNeraca = async (page: number = 1, limit: number = 2, kabupatenId: string) => {
        try {
            const response = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_HOST}/api/kecamatan?kabupaten_id=${kabupatenId}`, {
                headers: {
                    'content-type': 'application/json',
    ...
    const handleCheckboxChangeCommodity = (value: number) => { // Ubah tipe value ke number
    ...
    const handleSubmitHargaPangan = () => {
        ...
        if (date) {
            ...
            const checkedKeys = Object.keys(checkboxState)
                ...
                .join(',');
            console.log(checkedKeys, selectedKabupaten, selectedKecamatan, start_date, end_date);

            getHargaPanganData(1, 2, start_date as any, end_date as any, checkedKeys, selectedKabupaten, selectedKecamatan);
        }
                                    ...
                                    <div className="rounded-lg border border-stroke bg-white px-5  py-10 shadow-default">
                                        ...
                                        <div className="mt-5">
                                            ...
                                            <select
                                                id="location"
                                                name="location"
                                                onChange={(e) => setSelectedKecamatan((e.target as HTMLSelectElement).value)}
                                                className="mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6"
                                            >
                                        ...
                                        <div className="mt-5">
                                            ...
                                            {selectedCommodityOption.map((option) => (
                                                <CheckboxOne key={option.value} text={option.label} name={'komoditas'} value={option.value} checked={checkboxState[option.value] || false} // Use checkboxState to determine if checked
                                                    onChange={() => handleCheckboxChangeCommodity(option.value)} />
                                            ))}
                                        </div>
                                        <button onClick={handleSubmitHargaPangan} className="mt-5 bg-[#37B5FE] w-full inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white  hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                            Submit
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <div className=" bg-white rounded-lg border border-stroke bg-white px-5  py-10 shadow-default">
                                        ...
                                        <div className="mt-5">
                                            ...
                                            <select
                                                id="location"
                                                name="location"
                                                onChange={(e) => setSelectedKecamatanNeraca((e.target as HTMLSelectElement).value)}
                                                className="mt-2 block w-full rounded-md border-0 py-1.5 pl-3 pr-10 text-gray-900 ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-indigo-600 sm:text-sm sm:leading-6"
                                            >
                                                <option>Kecamatan</option>
                                                {selectedKecamatanOption.map((option) => (
                                        ...
                                        <div className="mt-5">
                                            ...
                                            {selectedCommodityOptionNeraca.map((option) => (
                                                <CheckboxOne key={option.value} text={option.label} name={'komoditas-neraca'} value={option.value} checked={checkboxStateNeraca[option.value] || false} // Use checkboxState to determine if checked
                                                    onChange={() => handleCheckboxChangeCommodityNeraca(option.value)} />
                                            ))}
                                        </div>
                                        ...
                                        <div className="">
                                            ...
                                            <div className="mt-2">
                                                <CheckboxOne key={'kebutuhan'} text={'kebutuhan'} name={'showHideHeader'} value={'kebutuhan'} checked={showHeader.kebutuhan} onChange={() => handleCheckboxChange('kebutuhan')} />
                                            </div>
                                            <div className="mt-2">
                                                <CheckboxOne key={'ketersediaan'} text={'ketersediaan'} name={'showHideHeader'} value={'ketersediaan'} checked={showHeader.ketersediaan} onChange={() => handleCheckboxChange('ketersediaan')} />
                                            </div>
                                            <div className="mt-2">
                                                <CheckboxOne key={'defisit'} text={'defisit'} name={'showHideHeader'} value={'defisit'} checked={showHeader.defisit} onChange={() => handleCheckboxChange('defisit')} />
                                            </div>
                                        </div>
                                        <button onClick={handleSubmitNeracaPangan} className="mt-5 bg-[#37B5FE] w-full inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white  hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                                            Submit
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div className="col-span-3 rounded-lg border border-stroke bg-white px-5  py-10 shadow-default">
                        ...
                        <div className={`w-full  ${activeTab === 'dashboard' ? '' : 'hidden'}`}>
                            ...
                            <div className="flex items-center py-4">
                                <Input
                                    placeholder="Filter commodities..."
                                    value={(tableNeraca.getColumn("commodity")?.getFilterValue() as string) ?? ""}
                                    onChange={(event) =>
                                        tableNeraca.getColumn("commodity")?.setFilterValue(event.target.value)
                                    }
                                    className="max-w-sm"
                                />
                                <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end">
                                        {tableNeraca.getAllColumns().filter((column) => column.getCanHide()).map((column) => (
                                            <DropdownMenuCheckboxItem
                                                key={column.id}
                                                className="capitalize"
                                                checked={column.getIsVisible()}
                                                ...
                                                <TableRow key={headerGroup.id}>
                                                    {headerGroup.headers.map((header, index) => (
                                                        <TableHead
                                                            ...
                                                            colSpan={header.id === 'commodity' ? 1 : Object.values(showHeader).filter(Boolean).length} // Mengatur rowSpan berdasarkan kondisi
                                                        >
                                                ...
                                                <TableRow key={`${headerGroup.id}-subheader`}>
                                                    {headerGroup.headers.map((header, index) => (
                                                        index < headerGroup.headers.length - 1 && ( // Memastikan hanya iterasi hingga panjang - 1
                                                            <>
                                                                <TableHead style={showHeader.kebutuhan ? { border: '1px solid black' } : { border: '1px solid black', display: 'none' }} key={`${header.id}-kebutuhan`}>
                                                                    Kebutuhan
                                                                </TableHead>
                                                                <TableHead style={showHeader.ketersediaan ? { border: '1px solid black' } : { border: '1px solid black', display: 'none' }} key={`${header.id}-ketersediaan`}>
                                                                    Ketersediaan
                                                                </TableHead>
                                                                <TableHead style={showHeader.defisit ? { border: '1px solid black' } : { border: '1px solid black', display: 'none' }} key={`${header.id}-defisit`}>
                                                                    Defisit
                                                                </TableHead>
                                                            </>
                                                        )
                                                ...
                                                <TableRow key={row.id} data-state={row.getIsSelected() && "selected"}>
                                                    ...
                                                    </TableCell>
                                                    {
                                                        row.original.dates && Object.keys(row.original.dates).map((date) => (
                                                            <>
                                                                <TableCell style={showHeader.kebutuhan ? { border: '1px solid black' } : { border: '1px solid black', display: 'none' }} className="text-right">
                                                                    {row.original.dates[date].kebutuhan}
                                                                </TableCell>
                                                                <TableCell style={showHeader.ketersediaan ? { border: '1px solid black' } : { border: '1px solid black', display: 'none' }} className="text-right">
                                                                    {row.original.dates[date].ketersediaan}
                                                                </TableCell>
                                                                <TableCell style={showHeader.defisit ? { border: '1px solid black' } : { border: '1px solid black', display: 'none' }} className="text-right">
